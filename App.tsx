...
const App: React.FC = () => {
  ...
  const menuItems = useMemo(() => [
    ...
    { id: 'privacy' as const, label: 'Seguridad y privacidad', icon: ShieldCheck }
  ], []);
        ...
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-4 py-8 space-y-1.5 overflow-y-auto custom-scroll">
            {menuItems.map(item => (
              ...
              }} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${activeTab === item.id ? 'bg-emerald-500 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-20 bg-white border-b border-slate-200 flex items-center px-4 md:px-8 no-print shrink-0">
          ...
          <div className="flex-1 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => { setEditingOrder(null); setActiveTab('register'); }} className="flex items-center gap-2 px-4 md:px-5 py-2.5 bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-widest rounded-2xl hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20 active:scale-95">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-10 custom-scroll">
          <div className="max-w-[1400px] mx-auto">
            ...
            {activeTab === 'register' && <OrderForm onSave={handleSaveOrder} customers={customers} orders={orders} editingOrder={editingOrder} onCancel={() => { setEditingOrder(null); setActiveTab('orders'); }} />}
