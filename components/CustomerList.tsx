...
const CustomerList: React.FC<CustomerListProps> = ({ customers, orders, onSave, onNewOrder }) => {
            ...
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th onClick={() => handleSort('name')} className="px-8 py-5 cursor-pointer hover:bg-slate-100 transition-colors">
                  <div className="flex items-center text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Nombre <SortIcon field="name"/></div>
                </th>
                <th onClick={() => handleSort('orderCount')} className="px-8 py-5 cursor-pointer hover:bg-slate-100 transition-colors">
                  <div className="flex items-center text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Pedidos <SortIcon field="orderCount"/></div>
                </th>
                <th onClick={() => handleSort('createdAt')} className="px-8 py-5 cursor-pointer hover:bg-slate-100 transition-colors">
                  <div className="flex items-center text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Registro <SortIcon field="createdAt"/></div>
                </th>
                <th className="px-8 py-5 text-right text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Acciones</th>
              </tr>
            </thead>
              ...
              {filteredAndSortedCustomers.map(c => {
                ...
                return (
                  <tr key={c.id} className={`group transition-colors ${isSelected ? 'bg-indigo-50/30' : 'hover:bg-slate-50/50'}`}>
                    ...
                    <td className="px-8 py-5">
                      <div className="text-sm font-medium text-slate-600">{new Date(c.createdAt).toLocaleDateString()}</div>
                    </td>
