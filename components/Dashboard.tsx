  ...
  const filteredStats = useMemo(() => {
    const current = orders.filter(order => {
      ...
      if (timeFilter === 'semana') {
        return (
          order.pickupDate >= currentWeekRange.start &&
          order.pickupDate <= currentWeekRange.end
        );
      }
    ...
    return {
      ...
      received: current.filter(order => order.status === 'Recibido').length,
      inProgress: current.filter(order => order.status === 'En elaboración').length,
      portions: current.reduce((acc, order) => {
        return (
          acc +
          order.products.reduce((productAcc, product) => {
            return productAcc + Number(product.portions || 0);
          }, 0)
        );
      }, 0),
    };
  }, [orders, timeFilter, todayStr, currentWeekRange]);

  const todayOrders = useMemo(() => {
    return orders
      .filter(order => order.pickupDate === todayStr)
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200 flex gap-1">
          {(['hoy', 'semana', 'total'] as TimeFilter[]).map((filter) => (
            <button
              key={filter}
              onClick={() => setTimeFilter(filter)}
              className={`px-6 py-2 rounded-xl text-[10px] font-semibold capitalize transition-all tracking-wider ${timeFilter === filter ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-50'
                }`}
            >
              {filter === 'hoy' ? 'Hoy' : filter === 'semana' ? 'Esta Semana' : 'Histórico'}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] shadow-sm border border-slate-200 p-8 flex flex-col">
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              ...
              className="group text-indigo-600 font-semibold text-[10px] uppercase tracking-[0.2em] flex items-center gap-2"
            >
          ...
          <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scroll flex-1">
            {todayOrders.length > 0 ? todayOrders.map(order => (
              <div
                key={order.id}
                className="group flex items-center p-5 rounded-3xl bg-slate-50 border border-transparent hover:border-indigo-100 hover:bg-white transition-all duration-300 gap-4"
              >
                ...
                <div className={`hidden sm:flex px-4 py-2 rounded-xl text-[9px] font-semibold uppercase tracking-widest border shrink-0 ${order.status === 'Recibido' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' :
